...
impl TgResponse {
    pub fn to_unescaped_string(&self) -> String {
        match self {
            ...
            Self::SuccessPeriodicInsert(reminder_str) => {
                format!("Added a periodic reminder:\n{}", reminder_str)
            }
            ...
            Self::NoChosenTimezone => {
                "You've not selected timezone yet, please do it with /settimezone command"
                    .to_owned()
            }
...
pub async fn send_message(text: &str, bot: &Bot, user_id: ChatId) -> Result<(), RequestError> {
